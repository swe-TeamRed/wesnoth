	...
	, fake_unit_()
{
	...
	for(const unit_const_ptr & recall_unit : resources::gameboard->get_team(side_number()).recall_list())
	{
...
{
	team & current_team = resources::gameboard->get_team(side_number());
...
void recall::apply_temp_modifier(unit_map& unit_map)
{
	...
	unit_ptr it = resources::gameboard->get_team(side_number()).recall_list().extract_if_matches_id(temp_unit_->id());
	assert(it);

	//Add cost to money spent on recruits.
	int cost = resources::gameboard->get_team(side_number()).recall_cost();
	if (it->recall_cost() > -1) {
		cost = it->recall_cost();
	}
	...
	resources::gameboard->get_team(side_number()).get_side_actions()->change_gold_spent_by(cost);
...
void recall::remove_temp_modifier(unit_map& unit_map)
{
	...
	resources::gameboard->get_team(side_number()).recall_list().add(temp_unit_);
}
...
void recall::draw_hex(map_location const& hex)
{
	if (hex == recall_hex_)
	{
		...
		if (cost < 0) {
			number_text << utils::unicode_minus << resources::gameboard->get_team(side_number()).recall_cost();
		}
		else {
...
action::error recall::check_validity() const
{
	...
	if( !resources::gameboard->get_team(side_number()).recall_list().find_if_matches_id(temp_unit_->id()) ) {
		return UNIT_UNAVAILABLE;
	}
	//Check that there is still enough gold to recall this unit
	if(resources::gameboard->get_team(side_number()).recall_cost() > resources::gameboard->get_team(side_number()).gold()) {
		return NOT_ENOUGH_GOLD;
	}
	//Check that there is a leader available to recall this unit
	if(!find_recruiter(side_number(),get_recall_hex())) {
		return NO_LEADER;
	}
